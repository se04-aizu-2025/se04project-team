...
| `last_updated` | 最終更新日（ISO 8601） | `2026-01-16` |
...
| 変更種別 | バージョン変更 | 例 |
|----------|---------------|-----|
| **破壊的変更** (構造変更、セクション削除) | MAJOR +1 | `1.0.0` → `2.0.0` |
| **機能追加** (新セクション追加) | MINOR +1 | `1.0.0` → `1.1.0` |
| **修正** (誤字、リンク修正) | PATCH +1 | `1.0.0` → `1.0.1` |

> [!IMPORTANT]
> `last_updated` は**すべての変更時**に更新必須。`version` は**MINOR以上の変更時**のみ更新。
...
ドキュメント内のコード例が実際のコードと一致していることを示すために、以下のコメントを追加します：

```kotlin
// 📁 path/to/file.kt (検証済み: YYYY-MM-DD)
fun example() { ... }
```

### 検証プロセス

1. コード例を書く際に、実際のソースファイルを参照
2. コメントに「検証済み: 日付」を追加
3. ドキュメント更新時に、参照先ファイルと整合性を確認
...
| 種類 | 形式 | 例 |
|------|------|-----|
| **公式ドキュメント** | `[名前](URL)` | `[Kotlin](https://kotlinlang.org)` |
| **参照リソース** | テーブル形式 | 下記参照 |
...
> [!WARNING]
> 外部URLは変更される可能性があります。リンク切れを発見した場合は、Internet Archive (archive.org) で代替を検索してください。

---
title: 開発計画
version: 1.2.0
last_updated: 2026-01-17
maintainer: Team
---

# Development Plan / 開発計画

このドキュメントでは、ソートアルゴリズム教育ツールの開発計画を記述します。各PRは小さな単位に分割し、レビューしやすいコード量を維持します。

## 目次

- [1. 開発方針](#1-開発方針)
- [2. PR一覧（時系列順）](#2-pr一覧時系列順)
- [3. Phase 1: 基盤整備](#3-phase-1-基盤整備)
- [4. Phase 2: アルゴリズム実装](#4-phase-2-アルゴリズム実装)
- [5. Phase 3: Data層実装](#5-phase-3-data層実装)
- [6. Phase 4: Design System構築](#6-phase-4-design-system構築)
- [7. Phase 5: Presentation層アーキテクチャ](#7-phase-5-presentation層アーキテクチャ)
- [8. Phase 6: Sort機能実装](#8-phase-6-sort機能実装)
- [9. Phase 7: アプリケーション構造構築](#9-phase-7-アプリケーション構造構築)
- [10. Phase 8: 可視化機能強化](#10-phase-8-可視化機能強化)
- [11. Phase 9: テスト・品質保証](#11-phase-9-テスト品質保証)
- [12. Phase 10: CUI実装](#12-phase-10-cui実装)
- [13. Phase 11: リリース準備](#13-phase-11-リリース準備)
- **PR-51~100: v2.0 拡張機能**
- [14. Phase 12: Learn機能](#14-phase-12-learn機能---アルゴリズム百科事典)
- [15. Phase 13: Compare機能](#15-phase-13-compare機能---アルゴリズム比較)
- [16. Phase 14: ゲーミフィケーション](#16-phase-14-ゲーミフィケーション)
- [17. Phase 15: 追加アルゴリズム](#17-phase-15-追加アルゴリズム)
- [18. Phase 16: 可視化強化](#18-phase-16-可視化強化)
- [19. Phase 17: 国際化 (i18n)](#19-phase-17-国際化-i18n)
- [20. Phase 18: データ永続化](#20-phase-18-データ永続化)
- [21. Phase 19: クイズ成績・スコア推移](#21-phase-19-クイズ成績スコア推移)
- [22. Phase 20: 品質・最適化](#22-phase-20-品質最適化)
- [23. Phase 21: v2.0 リリース](#23-phase-21-v20-リリース)

---

## 1. 開発方針

### 1.0 アーキテクチャ方針 (最適解)

- **DDD**: Domain層はエンティティ/ユースケース/Repository(Port)を中心に設計
- **Clean Architecture**: 依存方向は内側（Domain）へ固定
- **Hexagonal Architecture**: Data層はAdapterとして外部依存を隔離
- **Feature指向のData分割**: Data層はFeature単位 + Adapter/DataSource/Mapper/Policyで分割

### 1.1 PR サイズガイドライン

| サイズ | 変更行数 | レビュー時間目安 |
|--------|----------|------------------|
| 🟢 XS | ~50行 | 5分 |
| 🟢 S | ~100行 | 15分 |
| 🟡 M | ~200行 | 30分 |
| 🟠 L | ~400行 | 1時間 |
| 🔴 XL | 400行+ | 避ける |

**目標**: 1PR あたり **100-200行** を目安とする

### 1.2 ブランチ命名規則

```
feature/{PR番号}
```

**例**:
- `feature/01`
- `feature/25`

### 1.3 依存関係の表記

```
[PR-XX] → [PR-YY]  # PR-XX が完了後に PR-YY を開始
[PR-XX] ⇄ [PR-YY]  # 並行作業可能
```

---

## 2. PR一覧（時系列順）

### Phase 1: 基盤整備 (5 PRs) [完了✅]

| PR # | タスク | ブランチ | サイズ | 依存 |
|------|--------|----------|--------|------|
| PR-01 | SortType enum 拡張 | `feature/01` | 🟢 XS | - |
| PR-02 | BaseSortAlgorithm 作成 | `feature/02` | 🟡 M | PR-01 |
| PR-03 | BubbleSort リファクタリング | `feature/03` | 🟢 S | PR-02 |
| PR-04 | SortAlgorithmFactory 作成 | `feature/04` | 🟢 S | PR-03 |
| PR-05 | ExecuteSortUseCase リファクタリング | `feature/05` | 🟢 XS | PR-04 |

### Phase 2: アルゴリズム実装 (6 PRs) [完了✅]

| PR # | タスク | ブランチ | サイズ | 依存 |
|------|--------|----------|--------|------|
| PR-06 | SelectionSort 実装 | `feature/06` | 🟢 S | PR-05 |
| PR-07 | InsertionSort 実装 | `feature/07` | 🟢 S | PR-05 |
| PR-08 | ShellSort 実装 | `feature/08` | 🟡 M | PR-07 |
| PR-09 | MergeSort 実装 | `feature/09` | 🟡 M | PR-05 |
| PR-10 | QuickSort 実装 | `feature/10` | 🟡 M | PR-05 |
| PR-11 | HeapSort 実装 | `feature/11` | 🟡 M | PR-05 |

### Phase 3: Data層実装 (4 PRs) [完了✅]

| PR # | タスク | ブランチ | サイズ | 依存 |
|------|--------|----------|--------|------|
| PR-12 | ArrayGeneratorType enum | `feature/12` | 🟢 XS | PR-05 |
| PR-13 | ArrayGenerator interface | `feature/13` | 🟢 XS | PR-12 |
| PR-14 | ArrayGeneratorImpl 実装 | `feature/14` | 🟢 S | PR-13 |
| PR-15 | GenerateArrayUseCase | `feature/15` | 🟢 S | PR-14 |

### Phase 4: Design System構築 (8 PRs) [完了✅]

| PR # | タスク | ブランチ | サイズ | 依存 |
|------|--------|----------|--------|------|
| PR-16 | designsystem モジュール作成 | `feature/16` | 🟢 XS | - |
| PR-17 | ColorTokens 定義 | `feature/17` | 🟢 XS | PR-16 |
| PR-18 | SpacingTokens 定義 | `feature/18` | 🟢 XS | PR-16 |
| PR-19 | AnimationTokens 定義 | `feature/19` | 🟢 XS | PR-16 |
| PR-20 | Theme/Color/Typography | `feature/20` | 🟢 S | PR-17 |
| PR-21 | SortBar Atom | `feature/21` | 🟢 S | PR-17, PR-20 |
| PR-22 | SortButton/SortSlider Atoms | `feature/22` | 🟢 S | PR-20 |
| PR-23 | ArrayBar + Top/BottomBar + FloatingToken | `feature/23` | 🟢 S | PR-21 |

### Phase 5: Presentation層アーキテクチャ (3 PRs)

| PR # | タスク | ブランチ | サイズ | 依存 |
|------|--------|----------|--------|------|
| PR-24 | SortIntent 定義 | `feature/24` | 🟢 XS | PR-23 |
| Refactor | Presentation層モジュール分割 (Refactor) | `refactor` | 🟡 M | PR-24 |
| **PR-25** | **Commonモジュール実装** | `feature/25` | 🟢 S | Refactor |
| PR-26 | Navigationモジュール基盤 | `feature/26` | 🟢 S | PR-25 |
| PR-27 | アプリエントリポイント統合 | `feature/27` | 🟢 S | PR-26 |

**(備考: モジュール分割リファクタリングは PR-25 開始前に完了済み)**

### Phase 6: Sort機能実装 (6 PRs)

| PR # | タスク | ブランチ | サイズ | 依存 |
|------|--------|----------|--------|------|
| PR-28 | Sort機能：モジュールセットアップ | `feature/28` | 🟢 S | PR-27 |
| PR-29 | Sort機能：ドメイン統合 | `feature/29` | 🟢 S | PR-28 |
| PR-30 | Sort機能：ロジック (MVI) | `feature/30` | 🟡 M | PR-29 |
| PR-31 | Sort機能：UIコンポーネント | `feature/31` | 🟡 M | PR-30 |
| PR-32 | Sort機能：ビジュアライザ統合 | `feature/32` | 🟡 M | PR-31 |
| PR-33 | Sort機能：画面統合 | `feature/33` | 🟢 S | PR-32 |

### Phase 7: アプリケーション構造構築 (3 PRs)

| PR # | タスク | ブランチ | サイズ | 依存 |
|------|--------|----------|--------|------|
| PR-34 | Home機能実装 | `feature/34` | 🟢 S | PR-27 |
| PR-35 | Settings機能実装 | `feature/35` | 🟢 S | PR-27 |
| PR-36 | Learn/Compareプレースホルダー + Home/Sortレイアウト改善 | `feature/36` | 🟢 S | PR-27 |

### Phase 8: 可視化機能強化 (4 PRs)

| PR # | タスク | ブランチ | サイズ | 依存 |
|------|--------|----------|--------|------|
| PR-37 | 自動再生・一時停止ロジック | `feature/37` | 🟡 M | PR-33 |
| PR-38 | 再生コントロールUI | `feature/38` | 🟢 S | PR-37 |
| PR-39 | メトリクス・説明表示 | `feature/39` | 🟢 S | PR-33 |
| PR-40 | ステップ実行・速度調整 | `feature/40` | 🟢 S | PR-37 |

**補足**: Sort画面のメトリクス/レイアウト改善は PR-36 に前倒し対応済み。

### Phase 9: テスト・品質保証 (5 PRs)

| PR # | タスク | ブランチ | サイズ | 依存 |
|------|--------|----------|--------|------|
| PR-41 | アルゴリズム単体テスト | `feature/41` | 🟢 S | Phase 2 |
| PR-42 | Generator/UseCaseテスト | `feature/42` | 🟢 S | Phase 3 |
| PR-43 | ViewModel単体テスト | `feature/43` | 🟡 M | Phase 6 |
| PR-44 | UIコンポーネントテスト | `feature/44` | 🟢 S | Phase 6 |
| PR-45 | E2E/GUI動作確認 | `feature/45` | 🟢 S | Phase 8 |

**補足**: UIテストには浮遊バー/BottomBarの表示制御も追加対象。

### Phase 10: CUI実装 (4 PRs)

| PR # | タスク | ブランチ | サイズ | 依存 |
|------|--------|----------|--------|------|
| PR-46 | CLIエントリポイント | `feature/46` | 🟢 S | Phase 3 |
| PR-47 | CLI引数パーサー | `feature/47` | 🟢 S | PR-46 |
| PR-48 | CLI対話モード | `feature/48` | 🟡 M | PR-47 |
| PR-49 | CLI詳細出力機能 | `feature/49` | 🟢 S | PR-48 |

### Phase 11: ドキュメント整備・リファクタリング (1 PR)

| PR # | タスク | ブランチ | サイズ | 依存 |
|------|--------|----------|--------|------|
| PR-50 | ドキュメント整備・品質改善 | `feature/50` | 🟢 S | PR-49 |

---

## 3. Phase 1: 基盤整備

### PR-01: SortType enum 拡張

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/01` |
| **サイズ** | 🟢 XS (~20行) |
| **依存** | なし |

**変更内容**:
```kotlin
enum class SortType(val displayName: String) {
    BUBBLE("Bubble Sort"),
    SELECTION("Selection Sort"),
    INSERTION("Insertion Sort"),
    SHELL("Shell Sort"),
    MERGE("Merge Sort"),
    QUICK("Quick Sort"),
    HEAP("Heap Sort"),
}
```

---

### PR-02: BaseSortAlgorithm 作成

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/02` |
| **サイズ** | 🟡 M (~150行) |
| **依存** | PR-01 |

**新規ファイル**:
- `domain/src/commonMain/kotlin/dotnet/sort/algorithm/BaseSortAlgorithm.kt`

**変更内容**:
- `SortAlgorithm` インターフェース実装
- `snapshots`, `comparisonCount`, `swapCount` の管理
- `compare`, `swap` ヘルパーメソッド実装

---

### PR-03: BubbleSort リファクタリング

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/03` |
| **サイズ** | 🟢 S (~80行) |
| **依存** | PR-02 |

**変更ファイル**:
- `domain/src/commonMain/kotlin/dotnet/sort/algorithm/BubbleSortAlgorithm.kt`

**変更内容**:
- `BaseSortAlgorithm` を継承するように変更
- 重複コードを削除

---

### PR-04: SortAlgorithmFactory 作成

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/04` |
| **サイズ** | 🟢 S (~50行) |
| **依存** | PR-03 |

**新規ファイル**:
- `domain/src/commonMain/kotlin/dotnet/sort/algorithm/SortAlgorithmFactory.kt`

**変更内容**:
- `SortType` に応じて適切な `SortAlgorithm` インスタンスを返す Factory

---

### PR-05: ExecuteSortUseCase リファクタリング

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/05` |
| **サイズ** | 🟢 XS (~20行) |
| **依存** | PR-04 |

**変更ファイル**:
- `domain/src/commonMain/kotlin/dotnet/sort/usecase/ExecuteSortUseCase.kt`

**変更内容**:
- `SortAlgorithmFactory` を使用するように変更

---

## 4. Phase 2: アルゴリズム実装

### PR-06: SelectionSort 実装

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/06` |
| **サイズ** | 🟢 S (~80行) |
| **依存** | PR-05 |

**新規ファイル**:
- `domain/src/commonMain/kotlin/dotnet/sort/algorithm/SelectionSortAlgorithm.kt`

---

### PR-07: InsertionSort 実装

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/07` |
| **サイズ** | 🟢 S (~80行) |
| **依存** | PR-05 |

**新規ファイル**:
- `domain/src/commonMain/kotlin/dotnet/sort/algorithm/InsertionSortAlgorithm.kt`

---

### PR-08: ShellSort 実装

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/08` |
| **サイズ** | 🟡 M (~120行) |
| **依存** | PR-07 |

**新規ファイル**:
- `domain/src/commonMain/kotlin/dotnet/sort/algorithm/ShellSortAlgorithm.kt`

---

### PR-09: MergeSort 実装

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/09` |
| **サイズ** | 🟡 M (~150行) |
| **依存** | PR-05 |

**新規ファイル**:
- `domain/src/commonMain/kotlin/dotnet/sort/algorithm/MergeSortAlgorithm.kt`

---

### PR-10: QuickSort 実装

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/10` |
| **サイズ** | 🟡 M (~150行) |
| **依存** | PR-05 |

**新規ファイル**:
- `domain/src/commonMain/kotlin/dotnet/sort/algorithm/QuickSortAlgorithm.kt`

---

### PR-11: HeapSort 実装

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/11` |
| **サイズ** | 🟡 M (~150行) |
| **依存** | PR-05 |

**新規ファイル**:
- `domain/src/commonMain/kotlin/dotnet/sort/algorithm/HeapSortAlgorithm.kt`

---

## 5. Phase 3: Data層実装

### PR-12: ArrayGeneratorType enum

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/12` |
| **サイズ** | 🟢 XS (~15行) |
| **依存** | PR-05 |

**新規ファイル**:
- `domain/src/commonMain/kotlin/dotnet/sort/generator/ArrayGeneratorType.kt`

---

### PR-13: ArrayGenerator interface

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/13` |
| **サイズ** | 🟢 XS (~20行) |
| **依存** | PR-12 |

**新規ファイル**:
- `domain/src/commonMain/kotlin/dotnet/sort/generator/ArrayGenerator.kt`

---

### PR-14: ArrayGeneratorImpl 実装

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/14` |
| **サイズ** | 🟢 S (~100行) |
| **依存** | PR-13 |

**新規ファイル**:
- `data/src/commonMain/kotlin/dotnet/sort/generator/ArrayGeneratorImpl.kt`

---

### PR-15: GenerateArrayUseCase

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/15` |
| **サイズ** | 🟢 S (~40行) |
| **依存** | PR-14 |

**新規ファイル**:
- `domain/src/commonMain/kotlin/dotnet/sort/usecase/GenerateArrayUseCase.kt`

---

## 6. Phase 4: Design System構築

### PR-16: designsystem モジュール作成

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/16` |
| **サイズ** | 🟢 XS (~30行) |
| **依存** | なし |

**新規/変更ファイル**:
- `presentation/designsystem/build.gradle.kts`
- `settings.gradle.kts`
- `presentation/build.gradle.kts`

---

### PR-17: ColorTokens 定義

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/17` |
| **サイズ** | 🟢 XS (~40行) |
| **依存** | PR-16 |

**新規ファイル**:
- `presentation/designsystem/src/commonMain/kotlin/dotnet/sort/designsystem/tokens/ColorTokens.kt`

---

### PR-18: SpacingTokens 定義

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/18` |
| **サイズ** | 🟢 XS (~25行) |
| **依存** | PR-16 |

**新規ファイル**:
- `presentation/designsystem/src/commonMain/kotlin/dotnet/sort/designsystem/tokens/SpacingTokens.kt`

---

### PR-19: AnimationTokens 定義

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/19` |
| **サイズ** | 🟢 XS (~20行) |
| **依存** | PR-16 |

**新規ファイル**:
- `presentation/designsystem/src/commonMain/kotlin/dotnet/sort/designsystem/tokens/AnimationTokens.kt`

---

### PR-20: Theme/Color/Typography

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/20` |
| **サイズ** | 🟢 S (~100行) |
| **依存** | PR-17 |

**新規ファイル**:
- `presentation/designsystem/src/commonMain/kotlin/dotnet/sort/designsystem/theme/Theme.kt`
- `presentation/designsystem/src/commonMain/kotlin/dotnet/sort/designsystem/theme/Color.kt`
- `presentation/designsystem/src/commonMain/kotlin/dotnet/sort/designsystem/theme/Typography.kt`

---

### PR-21: SortBar Atom

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/21` |
| **サイズ** | 🟢 S (~60行) |
| **依存** | PR-17, PR-20 |

**新規ファイル**:
- `presentation/designsystem/src/commonMain/kotlin/dotnet/sort/designsystem/components/atoms/SortBar.kt`

---

### PR-22: SortButton/SortSlider Atoms

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/22` |
| **サイズ** | 🟢 S (~80行) |
| **依存** | PR-20 |

**新規ファイル**:
- `presentation/designsystem/src/commonMain/kotlin/dotnet/sort/designsystem/components/atoms/SortButton.kt`
- `presentation/designsystem/src/commonMain/kotlin/dotnet/sort/designsystem/components/atoms/SortSlider.kt`

---

### PR-23: ArrayBar Molecule

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/23` |
| **サイズ** | 🟢 S (~70行) |
| **依存** | PR-21 |

**新規ファイル**:
- `presentation/designsystem/src/commonMain/kotlin/dotnet/sort/designsystem/components/molecules/ArrayBar.kt`

---

## 7. Phase 5: Presentation層アーキテクチャ

本フェーズでは、SortIntentの定義から始まり、Presentation層の機能単位マルチモジュール化(Refactor)を経て、Commonモジュール等の基盤実装を進めます。

### PR-24: SortIntent 定義 [完了✅]

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/24` |
| **サイズ** | 🟢 XS |
| **依存** | なし |

**内容**:
- `SortIntent.kt` の定義
- ユーザー操作（ソート開始、リセット、ステップ実行等）のモデリング

---

### Refactor: Presentation層モジュール分割 [完了✅]

| 項目 | 内容 |
|------|------|
| **ブランチ** | `refactor` (または `main`/`develop` 直接) |
| **サイズ** | 🟡 M |
| **依存** | PR-24 |

**変更内容**:
- `presentation/common`, `presentation/navigation`, `presentation/feature/*` モジュール作成
- `SortIntent` を `presentation/feature/sort` へ移動
- 各モジュールの依存関係整理（Common <- Feature <- Navigation）

---

### PR-25: Commonモジュール実装

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/25` |
| **サイズ** | 🟢 S |
| **依存** | Refactor |

**実装内容**:
- `BaseViewModel`: コルーチンスコープ管理、エラーハンドリング共通化
- MVIインターフェース: `UnidirectionalViewModel`, `Intent`, `UiState`
- UI共通拡張関数: `Modifier` 拡張など

---

### PR-26: Navigationモジュール基盤

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/26` |
| **サイズ** | 🟢 S |
| **依存** | PR-25 |

**実装内容**:
- `Screen` シールドクラス/インターフェース: ルート定義（Type-safe Navigation）
- `dotnet.presentation.navigation.AppNavigation`: NavHostの実装基盤
- 各Featureモジュールへの画面コンポーザブル要求定義

---

### PR-27: アプリエントリポイント統合

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/27` |
| **サイズ** | 🟢 S |
| **依存** | PR-26 |

**実装内容**:
- `composeApp` から `presentation/navigation` への依存追加
- `App.kt` で `AppNavigation` (NavHost) を呼び出し
- アプリ起動時にとりあえず空白の画面または仮のホームが表示される状態にする

---

## 8. Phase 6: Sort機能実装

Sort機能を独立したFeatureモジュールとして実装します。

### PR-28: Sort機能：モジュールセットアップ

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/28` |
| **サイズ** | 🟢 S |
| **依存** | PR-27 |

**実装内容**:
- `presentation/feature/sort` の `build.gradle.kts` 依存確認
- Domain層、DesignSystem層へのアクセス確認
- 空の `SortScreen` を作成し、Navigationから表示できるようにする

---

### PR-29: Sort機能：ドメイン統合

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/29` |
| **サイズ** | 🟢 S |
| **依存** | PR-28 |

**実装内容**:
- `SortViewModel` 作成（空の実装）
- `ExecuteSortUseCase`, `GenerateArrayUseCase` をDIまたは手動注入で利用可能にする
- 基本的なDI構成

---

### PR-30: Sort機能：ロジック (MVI)

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/30` |
| **サイズ** | 🟡 M |
| **依存** | PR-29 |

**実装内容**:
- `SortIntent` の利用（PR-24ですでに定義済み、必要に応じて拡張）
- `SortState`: UI状態定義（配列データ、ハイライト位置、ソート中status）
- `SortViewModel`: Intentを受け取り、UseCaseを実行し、Stateを更新するロジック
- **注意**: まだUIは仮実装でOK。ログ等でロジック動作を確認。

---

### PR-31: Sort機能：UIコンポーネント

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/31` |
| **サイズ** | 🟡 M |
| **依存** | PR-30 |

**実装内容**:
- `AlgorithmSelector`: アルゴリズム選択ドロップダウン/ボタン
- `ControlPanel`: 実行/リセットボタン、サイズ変更スライダー
- `ArrayInputPanel`: 手動入力エリア
- Design Systemのコンポーネントを使用する

---

### PR-32: Sort機能：ビジュアライザ統合

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/32` |
| **サイズ** | 🟡 M |
| **依存** | PR-31 |

**実装内容**:
- `SortVisualizer`: 配列を描画するコンポーネント（DesignSystemの `ArrayBar` 等を利用）
- Stateの `highlightIndices` 等に基づいてアニメーションや色分けを反映

---

### PR-33: Sort機能：画面統合

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/33` |
| **サイズ** | 🟢 S |
| **依存** | PR-32 |

**実装内容**:
- `SortScreen` に全てのコンポーネントを配置
- ViewModelとUIの完全な接続
- 画面レイアウトの調整（レスポンシブ対応の基礎）

---

## 9. Phase 7: アプリケーション構造構築

### PR-34: Home機能実装

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/34` |
| **サイズ** | 🟢 S |
| **依存** | PR-27 |

**実装内容**:
- `presentation/feature/home` 実装
- アプリの顔となるホーム画面実装
- 各Featureへのナビゲーションカード/ボタン配置
- NavigationモジュールでのHomeルート定義

---

### PR-35: Settings機能実装

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/35` |
| **サイズ** | 🟢 S |
| **依存** | PR-27 |

**実装内容**:
- `presentation/feature/settings` 実装
- テーマ切り替え（Light/Dark）トグル
- アプリ情報表示
- 言語設定（将来用のプレースホルダー）

---

### PR-36: Learn/Compareプレースホルダー

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/36` |
| **サイズ** | 🟢 S |
| **依存** | PR-27 |

**実装内容**:
- `presentation/feature/learn`, `presentation/feature/compare` 実装
- 現在は機能未実装であることを伝えるプレースホルダー画面構成 ("Coming Soon" 等)
- Navigationルートの確保とボタン連携

---

## 10. Phase 8: 可視化機能強化

### PR-37: 自動再生・一時停止ロジック

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/37` |
| **サイズ** | 🟡 M |
| **依存** | PR-33 |

**実装内容**:
- ViewModelでのコルーチンを使用したタイマー/Delay制御
- `Pause`, `Resume` Intentの実装
- 再生速度係数のState管理
- `isPlaying` 状態の管理

---

### PR-38: 再生コントロールUI

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/38` |
| **サイズ** | 🟢 S |
| **依存** | PR-37 |

**実装内容**:
- 再生/一時停止ボタン（状態によるアイコン切り替え）
- シークバー（進行状況の表示とジャンプ）
- 速度調整スライダーの実装

---

### PR-39: メトリクス・説明表示

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/39` |
| **サイズ** | 🟢 S |
| **依存** | PR-33 |

**実装内容**:
- `MetricsDisplay`: 比較回数・スワップ回数・計算量情報の表示
- `DescriptionDisplay`: 現在のステップで何が起きているか（例: "index[2]とindex[3]を交換しました"）のテキスト表示
- 多言語対応の基礎

---

### PR-40: ステップ実行・速度調整

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/40` |
| **サイズ** | 🟢 S |
| **依存** | PR-37 |

**実装内容**:
- 「次のステップへ」「前のステップへ」ボタンの実装
- 一時停止中のみ有効化するUI制御
- 速度変更時のリアルタイム反映

---

## 11. Phase 9: テスト・品質保証

### PR-41: アルゴリズム単体テスト

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/41` |
| **サイズ** | 🟢 S |
| **依存** | Phase 2 |

**実装内容**:
- O(n²), O(n log n) 各アルゴリズムの単体テスト
- エッジケース（空配列、1要素、逆順、ソート済み）の網羅
- スナップショット記録の正確性確認

---

### PR-42: Generator/UseCaseテスト

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/42` |
| **サイズ** | 🟢 S |
| **依存** | Phase 3 |

**実装内容**:
- `ArrayGenerator` の各タイプ（Random, Ascending等）の生成結果テスト
- `GenerateArrayUseCase` の結合テスト

---

### PR-43: ViewModel単体テスト

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/43` |
| **サイズ** | 🟡 M |
| **依存** | Phase 6 |

**実装内容**:
- `SortViewModel` の各 Intent に対する State 遷移テスト
- TestCoroutineDispatcher を用いた時間経過テスト
- Mockを用いた UseCase 呼び出し確認

---

### PR-44: UIコンポーネントテスト

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/44` |
| **サイズ** | 🟢 S |
| **依存** | Phase 6 |

**実装内容**:
- Compose UI Test を用いたコンポーネント単体テスト
- SortBar, SortButton 等の表示・クリック動作確認

---

### PR-45: E2E/GUI動作確認

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/45` |
| **サイズ** | 🟢 S |
| **依存** | Phase 8 |

**実装内容**:
- アプリ全体の動作確認シナリオ作成
- ナビゲーション、ソート実行、設定変更の一連のフロー確認

---

## 12. Phase 10: CUI実装

### PR-46: CLIエントリポイント

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/46` |
| **サイズ** | 🟢 S |
| **依存** | Phase 3 |

**実装内容**:
- `composeApp` 内に CLI 用の `main` 関数を作成
- Gradle タスク `runCli` の追加

---

### PR-47: CLI引数パーサー

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/47` |
| **サイズ** | 🟢 S |
| **依存** | PR-46 |

**実装内容**:
- コマンドライン引数（`--algorithm`, `--size`, `--help` 等）の解析ロジック実装
- エラーハンドリングとヘルプメッセージ表示

---

### PR-48: CLI対話モード

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/48` |
| **サイズ** | 🟡 M |
| **依存** | PR-47 |

**実装内容**:
- 引数なし起動時の対話型メニュー実装
- アルゴリズム選択、サイズ入力を対話的に行う機能

---

### PR-49: CLI詳細出力機能

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/49` |
| **サイズ** | 🟢 S |
| **依存** | PR-48 |

**実装内容**:
- `--verbose` オプション時のステップごとの配列状態出力
- アスキーアートを用いた簡易的な可視化出力

---

## 13. Phase 11: ドキュメント整備・リファクタリング

### PR-50: ドキュメント整備・品質改善

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/50` |
| **サイズ** | 🟢 S |
| **依存** | PR-49 |

**実装内容**:
- ドキュメントの包括的な監査・更新
- `README.md` の更新
- `USER_GUIDE.md` の作成
- `CODING_GUIDELINES.md` の作成
- `DESIGN_SYSTEM.md` の作成
- 実装状況に合わせたドキュメント整合性の確保

---

# PR-51~100: v2.0 拡張機能

以下は基盤整備後の拡張機能開発計画です。**本計画は `feature/51` から順番に進める前提**で、追加の番号分岐は行いません。
要求定義を超える教育ツールとしての価値を高めます。

---

## 14. Phase 12: Learn機能 - アルゴリズム百科事典

### PR-51: Learn画面基盤

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/51` |
| **サイズ** | 🟡 M |
| **依存** | PR-50 |

**実装内容**:
- LearnScreen のプレースホルダーを本実装に置換
- アルゴリズム一覧を表示するリストUI
- LearnViewModel、LearnIntent、LearnState の作成
- Koin DIモジュールの設定

---

### PR-52: アルゴリズム詳細画面

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/52` |
| **サイズ** | 🟡 M |
| **依存** | PR-51 |

**実装内容**:
- AlgorithmDetailScreen の作成
- 一覧からのナビゲーション実装
- タブまたはセクション構造でコンテンツ表示
- 戻るボタンとナビゲーション

---

### PR-53: アルゴリズム歴史・背景コンテンツ

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/53` |
| **サイズ** | 🟢 S |
| **依存** | PR-52 |

**実装内容**:
- 各アルゴリズムの歴史情報データクラス作成
- バブルソート: 1956年 Edward Friend による命名
- クイックソート: 1960年 Tony Hoare 発明
- マージソート: 1945年 John von Neumann 発明
- その他アルゴリズムの背景情報

---

### PR-54: 動作原理・概念説明

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/54` |
| **サイズ** | 🟡 M |
| **依存** | PR-53 |

**実装内容**:
- 各アルゴリズムの動作原理をわかりやすく説明
- アニメーション付き図解（オプション）
- 「隣接要素を比較して交換」などの基本概念
- 分割統治法、ヒープ構造などの解説

---

### PR-55: 計算量詳細解説

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/55` |
| **サイズ** | 🟢 S |
| **依存** | PR-54 |

**実装内容**:
- 最良/平均/最悪ケースの計算量表示
- O記法の意味解説
- 空間計算量の説明
- なぜその計算量になるかの直感的説明

---

### PR-56: ユースケース・使用場面

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/56` |
| **サイズ** | 🟢 S |
| **依存** | PR-55 |

**実装内容**:
- 各アルゴリズムの最適な使用場面
- 「小さい配列には挿入ソート」など
- 「ほぼソート済みにはバブルソート」
- 実世界での使用例（Tim Sortはpython/java標準など）

---

### PR-57: Kotlin実装コード表示

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/57` |
| **サイズ** | 🟡 M |
| **依存** | PR-56 |

**実装内容**:
- シンタックスハイライト付きコード表示
- 各アルゴリズムのKotlin実装例
- コードコピー機能
- 行番号表示

---

### PR-58: 具体的動作例表示

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/58` |
| **サイズ** | 🟡 M |
| **依存** | PR-57 |

**実装内容**:
- 配列 [5, 3, 1, 4, 2] などでのステップ表示
- 各ステップの配列状態と説明
- 「可視化で見る」ボタンから SortScreen へ遷移
- 具体例による理解促進

---

## 15. Phase 13: Compare機能 - アルゴリズム比較

### PR-59: Compare画面基盤

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/59` |
| **サイズ** | 🟡 M |
| **依存** | PR-50 |

**実装内容**:
- CompareScreen のプレースホルダーを本実装に置換
- CompareViewModel、CompareIntent、CompareState の作成
- 2つのアルゴリズム選択UI
- Koin DIモジュールの設定

---

### PR-60: 並列実行エンジン

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/60` |
| **サイズ** | 🟡 M |
| **依存** | PR-59 |

**実装内容**:
- 同一配列で2つのアルゴリズムを同時実行
- 実行時間の同期制御
- メトリクスの個別収集
- CompareResult データクラス

---

### PR-61: 並列可視化UI

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/61` |
| **サイズ** | 🟡 M |
| **依存** | PR-60 |

**実装内容**:
- 2つの SortVisualizer を横に並べて表示
- 同期再生制御
- 個別の再生速度調整
- レスポンシブレイアウト

---

### PR-62: メトリクス比較表

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/62` |
| **サイズ** | 🟢 S |
| **依存** | PR-61 |

**実装内容**:
- 比較回数、交換回数、実行時間の比較表
- 勝敗表示（どちらが速いか）
- パーセンテージ差分表示
- グラフ可視化（バーチャート）

---

### PR-63: 入力パターン選択

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/63` |
| **サイズ** | 🟢 S |
| **依存** | PR-62 |

**実装内容**:
- Random/Sorted/Reverse/Nearly Sorted/Few Unique の選択
- パターンごとのアルゴリズム特性比較
- パターン説明ツールチップ

---

## 16. Phase 14: ゲーミフィケーション

**補足**: クイズ成績・スコア推移の永続化は Phase 19 で実施。

### PR-64: Quiz画面基盤

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/64` |
| **サイズ** | 🟡 M |
| **依存** | PR-50 |

**実装内容**:
- QuizScreen の新規作成
- QuizViewModel、QuizIntent、QuizState
- ホーム画面への「Quiz」メニュー追加
- ナビゲーション設定

---

### PR-65: ソート早押しクイズ機能

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/65` |
| **サイズ** | 🟠 L |
| **依存** | PR-64 |

**実装内容**:
- 現在の配列状態と次のステップ候補を表示
- ユーザーが「次にどの要素が交換されるか」を選択
- タイマー表示（早押し要素）
- 正解/不正解のフィードバック

---

### PR-66: 正誤判定ロジック

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/66` |
| **サイズ** | 🟢 S |
| **依存** | PR-65 |

**実装内容**:
- ユーザー選択と正解の比較
- 部分正解の判定（順序が逆でもOKなど）
- 正解時のアニメーション・効果音
- 不正解時の正解表示

---

### PR-67: スコアシステム

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/67` |
| **サイズ** | 🟢 S |
| **依存** | PR-66 |

**実装内容**:
- 正解数カウント
- 連続正解ボーナス
- 速度ボーナス（早く答えるほど高得点）
- スコア表示UI

---

### PR-68: アルゴリズム推測ゲーム

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/68` |
| **サイズ** | 🟡 M |
| **依存** | PR-67 |

**実装内容**:
- ソート過程のアニメーションを見せる
- 「これは何ソート？」を当てるゲーム
- 7つのアルゴリズムから選択
- ヒント機能（計算量など）

---

### PR-69: 難易度設定

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/69` |
| **サイズ** | 🟢 S |
| **依存** | PR-68 |

**実装内容**:
- Easy: 小さい配列、制限時間長め
- Medium: 中程度の配列
- Hard: 大きい配列、制限時間短め
- 難易度選択UI

---

### PR-70: クイズ結果サマリー

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/70` |
| **サイズ** | 🟢 S |
| **依存** | PR-69 |

**実装内容**:
- クイズ終了後の成績表示画面
- 正解率、総合スコア、最長連続正解
- 苦手なアルゴリズムの分析
- 「もう一度挑戦」ボタン
- **履歴保存/スコア推移は Phase 19 で追加**

---

### PR-71: ローカルランキング

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/71` |
| **サイズ** | 🟡 M |
| **依存** | PR-70 |

**実装内容**:
- ランキング表示画面
- 難易度別ランキング
- スコアリセット機能
- **保存処理は Phase 19 の永続化を利用**

---

## 17. Phase 15: 追加アルゴリズム

### PR-72: Counting Sort

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/72` |
| **サイズ** | 🟡 M |
| **依存** | PR-50 |

**実装内容**:
- CountingSortAlgorithm クラス実装
- 非比較ソートアルゴリズム
- 時間計算量: O(n+k)、空間計算量: O(k)
- SortType への追加、Factory更新
- 単体テスト作成

---

### PR-73: Radix Sort

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/73` |
| **サイズ** | 🟡 M |
| **依存** | PR-72 |

**実装内容**:
- RadixSortAlgorithm クラス実装
- 桁ごとにソートする手法
- 時間計算量: O(nk)、空間計算量: O(n+k)
- LSD (Least Significant Digit) 方式
- 単体テスト作成

---

### PR-74: Bucket Sort

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/74` |
| **サイズ** | 🟡 M |
| **依存** | PR-73 |

**実装内容**:
- BucketSortAlgorithm クラス実装
- 分布を利用したソート
- 時間計算量: O(n+k)（平均）
- バケット数の最適化
- 単体テスト作成

---

### PR-75: Tim Sort

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/75` |
| **サイズ** | 🟠 L |
| **依存** | PR-74 |

**実装内容**:
- TimSortAlgorithm クラス実装
- Python/Java 標準のハイブリッドソート
- マージソート + 挿入ソートの組み合わせ
- Run の検出と最適化
- 時間計算量: O(n log n)
- 単体テスト作成

---

### PR-76: Comb Sort

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/76` |
| **サイズ** | 🟢 S |
| **依存** | PR-75 |

**実装内容**:
- CombSortAlgorithm クラス実装
- バブルソートの改良版
- ギャップを徐々に縮小
- 時間計算量: O(n²/2^p)
- 単体テスト作成

---

### PR-77: Cocktail Sort

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/77` |
| **サイズ** | 🟢 S |
| **依存** | PR-76 |

**実装内容**:
- CocktailSortAlgorithm クラス実装
- 双方向バブルソート（シェーカーソート）
- 左右交互に走査
- 時間計算量: O(n²)
- 単体テスト作成

---

### PR-78: Gnome Sort

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/78` |
| **サイズ** | 🟢 S |
| **依存** | PR-77 |

**実装内容**:
- GnomeSortAlgorithm クラス実装
- シンプルな挿入系アルゴリズム
- 「庭師のソート」とも呼ばれる
- 時間計算量: O(n²)
- 単体テスト作成

---

### PR-79: Odd-Even Sort

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/79` |
| **サイズ** | 🟢 S |
| **依存** | PR-78 |

**実装内容**:
- OddEvenSortAlgorithm クラス実装
- 並列化に適したソート
- 奇数/偶数インデックスを交互に処理
- 時間計算量: O(n²)
- 単体テスト作成

---

### PR-80: Bogo Sort

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/80` |
| **サイズ** | 🟢 S |
| **依存** | PR-79 |

**実装内容**:
- BogoSortAlgorithm クラス実装
- 「最悪のソートアルゴリズム」として教育目的
- ランダムシャッフルして確認を繰り返す
- 時間計算量: O((n+1)!) 平均
- 実行制限（小さい配列のみ）
- 単体テスト作成

---

### PR-81: Bitonic Sort

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/81` |
| **サイズ** | 🟡 M |
| **依存** | PR-80 |

**実装内容**:
- BitonicSortAlgorithm クラス実装
- 並列ソートの入門的アルゴリズム
- Bitonic シーケンスの構築とマージ
- 時間計算量: O(n log²n)
- 単体テスト作成

---

## 18. Phase 16: 可視化強化

### PR-82: カスタムカラーテーマ

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/82` |
| **サイズ** | 🟢 S |
| **依存** | PR-50 |

**実装内容**:
- バーの色をユーザーが選択可能に
- プリセットテーマ（Kotlin, Ocean, Forest など）
- 設定画面への追加

---

### PR-83: アニメーション速度細調整

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/83` |
| **サイズ** | 🟢 S |
| **依存** | PR-82 |

**実装内容**:
- 0.1x～10x の速度範囲
- スライダーによる連続調整
- プリセットボタン（1x, 2x, 4x）

---

### PR-84: サウンドエフェクト

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/84` |
| **サイズ** | 🟡 M |
| **依存** | PR-83 |

**実装内容**:
- 比較時・交換時の音
- 音の高さで値を表現
- サウンドのON/OFF設定
- 音量調整

---

### PR-85: ソート完了アニメーション

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/85` |
| **サイズ** | 🟢 S |
| **依存** | PR-84 |

**実装内容**:
- ソート完了時の演出
- バーが順番に光る「ウェーブ」効果
- 完了メッセージ表示

---

## 19. Phase 17: 国際化 (i18n)

### PR-86: i18nインフラ整備

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/86` |
| **サイズ** | 🟡 M |
| **依存** | PR-50 |

**実装内容**:
- compose-resources によるリソース管理
- 言語切替のロジック
- 言語設定の永続化

---

### PR-87: 日本語リソース

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/87` |
| **サイズ** | 🟢 S |
| **依存** | PR-86 |

**実装内容**:
- UI文字列の日本語化
- アルゴリズム名の日本語表記
- ボタン、メニュー、説明文

---

### PR-88: 英語リソース

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/88` |
| **サイズ** | 🟢 S |
| **依存** | PR-87 |

**実装内容**:
- UI文字列の英語化
- デフォルト言語としての英語
- 国際ユーザー対応

---

### PR-89: 言語切替UI

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/89` |
| **サイズ** | 🟢 S |
| **依存** | PR-88 |

**実装内容**:
- 設定画面への言語選択追加
- ドロップダウンで言語選択
- 即時反映

---

### PR-90: Learn/Quiz多言語化

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/90` |
| **サイズ** | 🟡 M |
| **依存** | PR-89 |

**実装内容**:
- アルゴリズム説明文の多言語化
- クイズ問題文の多言語化
- 歴史・背景の多言語対応

---

## 20. Phase 18: データ永続化

**補足**: 成績・スコア推移の永続化は Phase 19 で扱う。

### PR-91: SQLDelight基盤

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/91` |
| **サイズ** | 🟡 M |
| **依存** | PR-50 |

**実装内容**:
- SQLDelight 導入とマルチプラットフォームDriver
- 基本スキーマ・Provider 基盤
- マイグレーション運用ルール整備

---

### PR-92: アルゴリズム履歴保存

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/92` |
| **サイズ** | 🟢 S |
| **依存** | PR-91 |

**実装内容**:
- アルゴリズム履歴の記録/取得
- Domain UseCase + Repository 連携

---

### PR-93: 学習進捗ダッシュボード

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/93` |
| **サイズ** | 🟡 M |
| **依存** | PR-92 |

**実装内容**:
- アルゴリズム別習熟度表示
- 総学習時間
- 推奨学習内容の提案

---

## 21. Phase 19: クイズ成績・スコア推移

### PR-94: クイズ履歴スキーマ

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/94` |
| **サイズ** | 🟡 M |
| **依存** | PR-92 |

**実装内容**:
- Quiz成績の保存テーブル追加
- SQLDelightクエリ定義
- Domainモデル/Repository(Port)追加
- 保存項目: 正解数/誤答数/連続正解/スコア/所要時間/難易度/出題アルゴリズム/日時/バージョン

---

### PR-95: 成績記録UseCase

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/95` |
| **サイズ** | 🟢 S |
| **依存** | PR-94 |

**実装内容**:
- 成績保存UseCase
- 成績取得UseCase
- Repository Adapter 実装

---

### PR-96: スコア推移グラフ

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/96` |
| **サイズ** | 🟡 M |
| **依存** | PR-95 |

**実装内容**:
- スコア推移表示UI
- 日次/週次/全期間の集計
- 可変期間フィルタ
- Learn/Quiz画面への統合

---

### PR-97: 学習ダッシュボード統合

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/97` |
| **サイズ** | 🟢 S |
| **依存** | PR-96 |

**実装内容**:
- Quiz結果サマリー連携
- 履歴からの分析表示
- 可視化UIの微調整

---

## 22. Phase 20: 品質・最適化

### PR-98: E2Eテスト

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/98` |
| **サイズ** | 🟡 M |
| **依存** | PR-97 |

**実装内容**:
- 主要ユーザーフローのテスト
- ソート実行からリセットまで
- 画面遷移テスト
- 永続化Repository/Mapper/SQLDelightクエリのテスト追加

---

### PR-99: パフォーマンス最適化

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/99` |
| **サイズ** | 🟡 M |
| **依存** | PR-98 |

**実装内容**:
- 大配列（1000要素以上）での描画最適化
- メモリ使用量の削減
- 再レンダリングの最小化

---

### PR-100: アクセシビリティ強化

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/100` |
| **サイズ** | 🟢 S |
| **依存** | PR-99 |

**実装内容**:
- キーボード操作の拡充
- スクリーンリーダー対応
- コントラスト調整

---

### PR-101: UI Polish

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/101` |
| **サイズ** | 🟢 S |
| **依存** | PR-100 |

**実装内容**:
- 細部のデザイン調整
- アニメーションの滑らかさ改善
- 不具合修正

---

## 23. Phase 21: v2.0 リリース

### PR-102: v2.0 ドキュメント更新

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/102` |
| **サイズ** | 🟢 S |
| **依存** | PR-101 |

**実装内容**:
- 新機能のドキュメント追加
- README の更新
- USER_GUIDE の拡充

---

### PR-103: v2.0 リリースビルド

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/103` |
| **サイズ** | 🟢 S |
| **依存** | PR-102 |

**実装内容**:
- 配布パッケージの作成
- 各プラットフォーム向けビルド
- 最終動作確認

---

### PR-104: 最終リファクタリング・コード品質向上

| 項目 | 内容 |
|------|------|
| **ブランチ** | `feature/104` |
| **サイズ** | 🟡 M |
| **依存** | PR-103 |

**実装内容**:
- コード全体のリファクタリング
- 未使用コードの削除
- パフォーマンス最終チューニング
- 技術的負債の解消
- 全ドキュメントの最終確認・更新

